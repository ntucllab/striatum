"""Decreasing-epsilon-greedy Algorithm under CBMA Setting
This module contains a class that implements decreasing-epsilon-greedy algorithm under CBMA setting.
"""
...
class DecreasingEpsilonGreedy(BaseCbma):
    """Decreasing-epsilon-greedy algorithm under cbma setting.
    ...
    self.explr_prob: float
        The probability of random exploration.
    rho: float
        The decreasing rate of epsilon.
    d: int
    ...
    def get_action(self, n_recommend, context):
        ...
        self.explr_prob /= (1 + self.rho)
        indicator = int(np.random.choice([1, 2], size=1, p=[self.explr_prob, 1 - self.explr_prob]))
        if indicator == 1:

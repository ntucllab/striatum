...
class TestLinUcb(unittest.TestCase):
    ...
    def test_model_storage(self):
        ...
        history_id, action = policy.get_action([1, 1])
        policy.reward(history_id, 1)
        self.assertTrue((policy._modelstorage._model['b'][action] == np.transpose(np.array([[1, 1]]))).all())
        self.assertTrue((policy._modelstorage._model['matrix_a'][action] == np.array([[2., 1.], [1., 2.]])).all())

    def test_delay_reward(self):
        policy = linucb.LinUCB(self.actions, self.historystorage,
                               self.modelstorage, 1.00, 2)
        history_id, action = policy.get_action([1, 1])
        history_id_2, action_2 = policy.get_action([0, 0])
        policy.reward(history_id, 1)
        self.assertTrue(
            (policy._historystorage.get_history(history_id).context == np.transpose(np.array([[1, 1]]))).all())
        self.assertTrue(
            (policy._historystorage.get_history(history_id_2).context == np.transpose(np.array([[0, 0]]))).all())
        self.assertEqual(policy._historystorage.get_history(history_id).reward, 1)
